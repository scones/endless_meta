...
namespace core {
  ...
  namespace support {
    ...
    struct duktape {
      ...
      static duktape create_from_peval_string(std::string const& source) {
        core::support::duktape duk;
      ...
      inline std::uint32_t peval_string(std::string const& input_string) {
        std::uint32_t result(duk_peval_string(m_context, input_string.c_str()));
        if (0 != result)
          throw core::error::duktape_error(std::string("peval failed:") + this->safe_to_string());
